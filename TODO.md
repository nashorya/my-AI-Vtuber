...
### [ ] P1-03 评估并优先移除应用内“虚拟麦克风”

**证据**

- `VirtualMicMixer` 支持 `WriteMic` 和 `WriteTts`，配置注释也声明混合真人麦克风与 TTS。
- `BotRuntime.StartAudio` 明确没有把真人麦克风写入 mixer，实际只输出 TTS。
- 该功能要求用户安装并理解 VB-Cable 等额外音频路由，且容易与直播软件、系统监听和真实麦克风形成回授或重复采集。
- 应用已经可以把 TTS 播放到指定输出设备；真人麦克风和 AI 语音的最终混音更适合由 OBS/直播软件显式管理。

**实施**

- 默认方案是删除 `VirtualMicMixer`、设置页开关、设备选择和运行时生命周期，不再在应用内混合真人麦克风与 TTS。
- 保留普通 TTS 输出设备选择；由 OBS/直播软件分别采集真人麦克风和 AI 语音输出，并在用户文档中给出最小音频路由示例。
- 为旧配置中的 `enable_virtual_mic` / `virtual_mic_device_name` 提供一次性迁移或兼容忽略，不能因未知字段导致配置加载失败，也不能在后续保存时重新启用已移除能力。
- 只有在真实用户场景证明外部混音无法满足需求后，才重新评估独立、受测试的音频路由模块；不得继续扩展当前半实现状态。

**验收**

- 不安装虚拟声卡时，应用仍可完成麦克风输入、AI TTS 播放、内录监听和 OBS/直播软件接入。
- 删除后没有残留的虚拟设备枚举、后台输出、事件订阅或配置热更新分支。
- 旧配置可安全升级；UI、配置模板、README 和实际音频路由一致。
- 使用推荐的 OBS/直播软件路由录制一轮真人麦克风 + AI TTS，验证声道、音量、静音、无重复采集和无回授。
...
### [ ] P1-08 防止对方主播声音经麦克风串音后被误归属为本地主播

**证据**

- 麦克风识别结果无条件套用 `MicTemplate`，内录识别结果无条件套用 `LoopbackTemplate`；系统只相信采集通道，没有验证实际说话人来源。
- 对方主播声音经扬声器外放后可能同时进入内录和真实麦克风；当前没有 AEC、双通道重复检测、说话人确认或声纹辅助。
- 麦克风路径优先级最高并会打断内录路径，因此一次串音不仅会被标记成“你的创造者说”，还可能取消正确的“对面说”处理。
- 当前 UI 只提示佩戴耳机，无法检测、解释或阻止误归属。

**实施**

- 依赖 `P0-04`：麦克风和内录帧使用同一单调时钟、source、generation 和有界输入协调器，先完成来源仲裁再进入 ASR/LLM。
- 默认推荐耳机和按进程内录；选择“全部音源”或检测到外放串音风险时，在 UI 给出明确状态和恢复建议。
- 保存短时内录参考窗口，对重叠的麦克风片段做音频相关性/指纹检测；与内录高度一致的麦克风副本应被抑制，由内录路径保留一次“对面说”事件。
- 评估并引入可自动验证的 AEC，以系统播放/内录信号作为 reference，在麦克风 VAD/ASR 前消除远端声音；若平台或设备不支持，必须显式降级，不能宣称已隔离。
- 可选的本地主播声纹只能作为辅助置信度，必须本地保存、可删除、可关闭；不能单独依赖声纹决定身份。
- 同时说话时不能简单静音整个麦克风通道；仲裁结果需要记录来源、重叠区间、置信度和降级原因，但默认不保存原始语音。

**验收**

- 播放录制的对方主播语音并以多档外放音量泄漏到麦克风时，只产生一次 `Loopback` 输入，不产生 `Microphone/创造者` 输入。
- 本地主播单独说话时仍稳定走麦克风路径；本地与对方重叠说话时，本地语音不会被整段误删，对方语音不会重复归属。
...
> 2026-09-10：连续控制实验切片见 [实验说明](docs/experiments/vts-continuous-control.md)。本轮不将 AVATAR 大项标记完成；实际 VTS 视觉验收另行记录。
