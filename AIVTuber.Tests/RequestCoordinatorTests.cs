...
{
    [Fact]
    public async Task AwaitedCompletion_IsIdleAndAcceptsNextLoopbackRequest()
    {
        await using var coordinator = new RequestCoordinator();
        for (var i = 0; i < 100; i++)
        {
            Assert.True(await coordinator.EnqueueAsync(InputSource.Loopback, async (_, _) => await Task.Yield()));
